...
def singlePull(rates, arankRate, srankRate, softten, pity):
    ...
    if   pity == 100:
        ...
        result = random.choices([b"S-rank Valkyrie", b"A-rank Valkyrie"], cum_weights=[srankRate, arankRate], k=1)
    else:
        # Regular pull
...
def resolvePull(pull, bRankPool, aRankPool, sRankPool):
    ...
    if b"Valk" in pull:
        ...
        elif b"A-rank" in pull:
            pull = random.choice(aRankPool)
        elif b"B-rank" in pull:
            pull = random.choice(bRankPool)

    if frag:
        pull["Name"] = pull["Name"] + b"frag"
        print("\t\t\tFragment")
...
bRankPool = [x for x in valkPool if x["Rank"] == b"B Rank"]
aRankPool = [x for x in valkPool if x["Rank"] == b"A Rank"]
sRankPool = [x for x in valkPool if x["Rank"] == b"S Rank"]
...
#while b"Frag" not in _result:
    ...
    if _result == b'A-rank Valkyrie':
        print("\tten-pull cleared")
        softten = 0
    elif _result == b'S-rank Valkyrie':
        print("\tpity reset")
        softten = 0
        pity = 0
